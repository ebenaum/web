...
import {
  BuildInputSelectValue,
  InputSelectValue,
  InputSelectConfig,
  WorldObjectsToChoices,
} from "./forms/input-select";
import {
  emptyTraits,
  InputTraitsValue,
  InputTraits,
...
> {
  constructor(props: CharacterFormProps) {
    ...
    this.state = {
      index: 0,
      traits: new InputTraitsValue(
        emptyTraits(this.props.characteristics),
        this.props.traitsPoints,
      ),
      ...
      classe: BuildInputSelectValue(
        [],
        WorldObjectsToChoices(props.characterClasses),
      ),
    };
  }
  ...
  form = (): any => {
    return {
      ...
      name:
        this.state.name.firstName.text + " " + this.state.name.lastName.text,
  ...
  races = (): WorldObject[] => {
    ...
    this.props.races.forEach((race: any) => {
      if (
        race.include === null ||
        race.include.length === 0 ||
        race.include.indexOf(faction) !== -1
      ) {
        races.push(race);
      }
    });
  ...
  buildForm = (): FormBuilderElement[] => {
    return [
      ...
      new InputSelectConfig(
        ...
        "TA FACTION ?",
        this.state.faction,
        WorldObjectsToChoices(this.props.factions),
        { alignment: "horizontal", multi: false, displayListNumber: true },
      ),
      new InputSelectConfig(
        "race",
        "TA RACE ?",
        this.state.race,
        WorldObjectsToChoices(this.races()),
        { alignment: "vertical", multi: false },
      ),
      new InputSelectConfig(
        "classe",
        "TA CLASSE ?",
        this.state.classe,
        WorldObjectsToChoices(this.props.characterClasses),
        { alignment: "horizontal", multi: true },
      ),
      new InputTraitsConfig(
        "traits",
        "CARAC ?",
        this.props.characteristics,
        this.state.traits.points,
        this.state.traits.traits,
        emptyTraits(this.props.characteristics),
      ),
    ];
  };
