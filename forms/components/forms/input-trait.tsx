...
export class InputTrait extends React.Component<InputTraitProps, any> {
  ...
  render() {
    ...
    const levels = this.props.levels.map((level, index) => {
      ...
      const isActive =
        levelInt >= this.minLevel() && levelInt <= this.maxLevel();
      return (
        <li key={index}>
          <div className="level">{levelString(index)}</div>
          <div
            className={`input-select-choice ${selected === index ? "selected" : ""} ${isActive ? "active" : ""} ${!isActive ? "disabled" : ""}`}
            onClick={this.onClick.bind(this, index)}
          >
            <div>
              {level.title} {checkmark}
            </div>
            <p
              className={`description ${level.description === undefined ? "hidden" : ""}`}
            >
              {level.description}
            </p>
          </div>
    ...
    return (
      <React.Fragment>
        <div className="q-select">
          <ul className="q-response-select">{levels}</ul>
        </div>
      </React.Fragment>
    );
  }
